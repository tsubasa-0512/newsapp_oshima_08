<?php 
  ...
  $url = 'https://relic.co.jp/battery/article';
  ...
  // タイトルスクレイピング
  foreach ($html->find('.archive-list .archive-list__item a .entry-title')as $ele) {
    $title =  $ele->plaintext;
    $title_array[] = $title;
  }
  
  // URLスクレイピング
  foreach ($html->find('.archive-list .archive-list__item a')as $ele) {
    $url_ele =  $ele->href;
    $url = $url_ele;
    $url_array[] = $url;
  }
