<?php 
  ...
  // タイトルスクレイピング
  foreach ($html->find('.m-panel-layout h3 span')as $ele) {
    $title =  $ele->plaintext;
    $title_array[] = $title;
  }
  
  // URLスクレイピング
  foreach ($html->find('.m-panel-layout a')as $ele) {
    $url_ele =  $ele->href;
    $url = 'https://signal.diamond.jp'.$url_ele;
    $url_array[] = $url;
  }
...
<head>
  ...
  <title>DIAMOND SIGNAL 新着記事</title>
</head>
<body>
  <aside>
